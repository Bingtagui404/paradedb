...
stmt = (
    select(
        ...
        MockItem.rating,
        func.pdb.agg(literal_column('\'{"value_count": {"field": "id"}}\'')).over().label("agg"),
    )
    .where(
...
stmt = (
    select(
        ...
        MockItem.rating,
        MockItem.category,
        func.pdb.agg(literal_column('\'{"value_count": {"field": "id"}}\''), literal_column("false"))
        .over()
        .label("agg"),
    )
    .where(
        MockItem.id.op("@@@")(func.pdb.all()),
        MockItem.description.op("|||")(literal_column("'running shoes'")),
    )
