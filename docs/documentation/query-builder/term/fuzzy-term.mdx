...
```python Django
from paradedb import FuzzyTerm, ParadeDB

MockItem.objects.filter(
    description=ParadeDB(FuzzyTerm('shoez'))
).values('description', 'rating', 'category')[:5]
```

```ruby Rails
MockItem.search(:description)
        .term("shoez", distance: 2)
        .select(:description, :rating, :category)
        .limit(5)
```
...
stmt = (
    ...
    .where(search.term(MockItem.description, "shoez", distance=2, transpose_cost_one=True))
    .limit(5)
)
