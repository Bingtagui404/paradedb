...
MockItem.objects.filter(
    description=ParadeDB(Match('ruining shoez', operator='OR', distance=2))
).values('description', 'rating', 'category')
```

```ruby Rails
MockItem.search(:description)
        .matching_any("ruining shoez", distance: 2)
        .select(:description, :rating, :category)
```
...
stmt = (
    select(MockItem.description, MockItem.rating, MockItem.category)
    .where(search.match_any(MockItem.description, "ruining shoez", distance=2))
)
